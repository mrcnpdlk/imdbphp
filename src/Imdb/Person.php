...
class Person extends MdbBase
{
    ...
    public function articles()
    {
        if (empty($this->pub_articles)) {
            $this->parsearticles($this->pub_articles, 'Publicity', 'Article');
        }
    ...
    public function bio()
    {
        if (empty($this->bio_bio)) {
            ...
            if (preg_match('!<h4 class="li_group">Mini Bio[^>]+?>(.+?)<(h4 class="li_group"|div class="article")!ims', $this->page['Bio'],
                $block)) {
                ...
                for ($i = 0; $i < count($matches[0]); ++$i) {
                    ...
                    $author          = 'Written by ' . str_replace('/search/name', 'http://' . $this->imdbsite . '/search/name',
                            $matches['author'][$i]);
     ...
     *
     * @param array where to store the filmography
    ...
    public function interviews()
    {
        if (empty($this->pub_interviews)) {
            $this->parsearticles($this->pub_interviews, 'Publicity', 'Interview');
        }
    ...
    public function magcovers()
    {
        if (empty($this->pub_magcovers)) {
            $this->parsearticles($this->pub_magcovers, 'Publicity', 'Magazine cover photo');
        }
    ...
    public function pictorials()
    {
        if (empty($this->pub_pictorials)) {
            $this->parsearticles($this->pub_pictorials, 'Publicity', 'Pictorial');
        }
    ...
    public function pubmovies()
    {
        if (empty($this->pub_movies)) {
            $this->parsepubmovies($this->pub_movies, 'Publicity', 'Biographical movies');
        }
    ...
    public function pubportraits()
    {
        if (empty($this->pub_portraits)) {
            $this->parsepubmovies($this->pub_portraits, 'Publicity', 'Portrayed in');
        }
    ...
    public function pubprints()
    {
        if (empty($this->pub_prints)) {
            ...
            $pc    = count($arr);
            for ($i = 1; $i < $pc; ++$i) {
                if (preg_match('/(.*).\s*<i>(.*)<\/i>\s*((.*):|)((.*),|)\s*((\d+)\.|)\s*ISBN\s*<a href="(.*)">(.*)<\/a>/iU', $arr[$i],
                    $match)) {
                    $this->pub_prints[] = [
                        'author'    => $match[1],
                        'title'     => $match[2],
                        'place'     => trim($match[4]),
                        'publisher' => trim($match[6]),
                        'year'      => $match[8],
                        'isbn'      => $match[10],
                        'url'       => $match[9],
                    ];
